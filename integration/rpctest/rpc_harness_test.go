...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt luxutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(luxutil.Amount(5 * luxutil.SatoshiPerBitcoin))
	...
	txid = genSpend(luxutil.Amount(500 * luxutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(luxutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*luxutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, luxutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := luxutil.Amount(250 * luxutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = luxutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := luxutil.Amount(50 * luxutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := luxutil.Amount(numMatureOutputs * 50 * luxutil.SatoshiPerBitcoin)
