...
import (
	...
	"github.com/bitbandi/luxd/integration/rpctest"
...
func makeTestOutput(r *rpctest.Harness, t *testing.T,
	amt luxutil.Amount) (*btcec.PrivateKey, *wire.OutPoint, []byte, error) {
	...
	// spend.
	a, err := luxutil.NewAddressPubKey(key.PubKey().SerializeCompressed(), r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0113Activation(t *testing.T) {
	...
	luxdCfg := []string{"--rejectnonstd"}
	r, err := rpctest.New(&chaincfg.SimNetParams, nil, luxdCfg)
	if err != nil {
	...
	const outputValue = luxutil.SatoshiPerBitcoin
	...
	txns := []*luxutil.Tx{luxutil.NewTx(tx)}
...
func createCSVOutput(r *rpctest.Harness, t *testing.T,
	numSatoshis luxutil.Amount, timeLock int32,
	...
	p2shAddr, err := luxutil.NewAddressScriptHash(csvScript, r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0068AndBIP0112Activation(t *testing.T) {
	...
	const (
		outputAmt         = luxutil.SatoshiPerBitcoin
	...
	for txVersion := int32(0); txVersion < 3; txVersion++ {
		...
		txns := []*luxutil.Tx{luxutil.NewTx(spendingTx)}
		block, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
		if err != nil {
			t.Fatalf("unable to submit block: %v", err)
		}
	...
	for i, test := range tests {
		...
		if !test.accept {
			txns := []*luxutil.Tx{luxutil.NewTx(test.tx)}
			_, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
			if err == nil {
