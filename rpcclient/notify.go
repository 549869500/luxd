...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *luxutil.Tx, details *btcjson.BlockDetails)
	...
	//
	// NOTE: Deprecated. Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *luxutil.Tx, details *btcjson.BlockDetails)
	...
	// result of a rescan request, due to how luxd may send various rescan
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount luxutil.Amount)
	...
	// such as btcwallet.
	OnAccountBalance func(account string, balance luxutil.Amount, confirmed bool)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	case btcjson.BtcdConnectedNtfnMethod:
		...
		if err != nil {
			log.Warnf("Received invalid luxd connected "+
				"notification: %v", err)
			return
		}
...
//
// NOTE: This is a luxd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*luxutil.Tx, error) {
	...
	transactions := make([]*luxutil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = luxutil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*luxutil.Tx,
	*btcjson.BlockDetails, error) {
	...
	return luxutil.NewTx(&msgTx), block, nil
}
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := luxutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
// parseBtcdConnectedNtfnParams parses out the connection status of luxd
// and btcwallet from the parameters of a luxdconnected notification.
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance luxutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := luxutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
//
// NOTE: This is a luxd extension and requires a websocket connection.
...
//
// NOTE: Deprecated. Use LoadTxFilterAsync instead.
func (c *Client) NotifyReceivedAsync(addresses []luxutil.Address) FutureNotifyReceivedResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
...
//
// NOTE: Deprecated. Use LoadTxFilter instead.
func (c *Client) NotifyReceived(addresses []luxutil.Address) error {
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	addresses []luxutil.Address, outpoints []*wire.OutPoint,
...
// NOTE: This is a luxd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []luxutil.Address,
...
// NOTE: This is a luxd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []luxutil.Address, outPoints []wire.OutPoint) error {
