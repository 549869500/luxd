...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: luxutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := luxutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*luxutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, luxutil.NewTx(tx))
	}
...
func additionalCoinbase(amount luxutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee luxutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee luxutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee luxutil.Amount) *wire.MsgTx {
