...
const (
	// svcName is the name of luxd service.
	svcName = "luxdsvc"
...
// logServiceStartOfDay logs information about luxd when the main server has
...
// luxdService houses the main service handler which handles all service
// updates and launching luxdMain.
type luxdService struct{}
...
// long-running luxdMain (which is the real meat of luxd), handles service
// change requests, and notifies the service control manager of changes.
func (s *luxdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start luxdMain in a separate goroutine so the service can start
...
// installService attempts to install the luxd service.  Typically this should
...
// removeService attempts to uninstall the luxd service.  Typically this should
...
// startService attempts to start the luxd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &luxdService{})
	if err != nil {
