...
const (
	defaultConfigFilename        = "luxd.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "luxd.log"
	defaultMaxPeers              = 125
	...
	sampleConfigFilename         = "sample-luxd.conf"
	defaultTxIndex               = false
	defaultAddrIndex             = false
)

var (
	defaultHomeDir     = luxutil.AppDataDir("luxd", false)
...
// config defines the configuration options for luxd.
//
...
//
// The above results in luxd functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.DisableRPC {
		luxdLog.Infof("RPC service is disabled")
	}
	...
	cfg.minRelayTxFee, err = luxutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]luxutil.Address, 0, len(cfg.MiningAddrs))
	for _, strAddr := range cfg.MiningAddrs {
		addr, err := luxutil.DecodeAddress(strAddr, activeNetParams.Params)
		if err != nil {
	...
	if configFileError != nil {
		luxdLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// createDefaultConfig copies the file sample-luxd.conf to the given destination path,
...
// luxdDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func luxdDial(addr net.Addr) (net.Conn, error) {
...
// luxdLookup resolves the IP of the given host using the correct DNS lookup
...
func luxdLookup(host string) ([]net.IP, error) {
