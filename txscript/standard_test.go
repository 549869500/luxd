...
// newAddressPubKey returns a new luxutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) luxutil.Address {
	addr, err := luxutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new luxutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) luxutil.Address {
	addr, err := luxutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new luxutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) luxutil.Address {
	addr, err := luxutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the luxutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the luxutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := luxutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := luxutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := luxutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := luxutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := luxutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*luxutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*luxutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*luxutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*luxutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*luxutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
