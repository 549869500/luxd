...
import (
	...
	rpc "github.com/bitbandi/luxd/rpcclient"
	"github.com/bitbandi/luxutil"
)

// nodeConfig contains all the args, and data required to launch a luxd process
...
// arguments returns an array of arguments that be used to launch the luxd
// process.
...
// command returns the exec.Cmd which will be used to start the luxd process.
...
// to the luxd process that is launched via Start().
...
// as the base for the log and data directories for luxd.
...
// start creates a new luxd process, and writes its pid in a file reserved for
...
// stop interrupts the running luxd process process, and waits until it exits
...
// shutdown terminates the running luxd process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := luxutil.NewTLSCertPair(org, validUntil, nil)
	if err != nil {
		return err
	}
