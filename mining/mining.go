...
const (
	...
	MinHighPriority = luxutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr luxutil.Address) (*luxutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *luxutil.Tx, height int32) error {
...
func logSkippedDeps(tx *luxutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress luxutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*luxutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := luxutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := luxutil.NewBlock(&msgBlock)
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A luxutil.Block should use saved in the state to avoid
