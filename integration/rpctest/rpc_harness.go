...
import (
	...
	"github.com/bitbandi/luxd/rpcclient"
...
// Harness fully encapsulates an active luxd process to provide a unified
// platform for creating rpc driven integration tests involving luxd. The
// active luxd node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active luxd process is fully
...
func New(activeNet *chaincfg.Params, handlers *rpcclient.NotificationHandlers,
	...
	if handlers.OnFilteredBlockConnected != nil {
		...
		handlers.OnFilteredBlockConnected = func(height int32, header *wire.BlockHeader, filteredTxns []*luxutil.Tx) {
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the luxd node itself. This spawns a new process which will be
	// managed
	...
	// wallet.
	filterAddrs := []luxutil.Address{h.wallet.coinbaseAddr}
	if err := h.Node.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure luxd properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created luxd
...
func (h *Harness) NewAddress() (luxutil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() luxutil.Amount {
...
func (h *Harness) SendOutputs(targetOutputs []*wire.TxOut,
	feeRate luxutil.Amount) (*chainhash.Hash, error) {
...
func (h *Harness) CreateTransaction(targetOutputs []*wire.TxOut,
	feeRate luxutil.Amount) (*wire.MsgTx, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) GenerateAndSubmitBlock(txns []*luxutil.Tx, blockVersion int32,
	blockTime time.Time) (*luxutil.Block, error) {
...
func (h *Harness) GenerateAndSubmitBlockWithCustomCoinbaseOutputs(
	txns []*luxutil.Tx, blockVersion int32, blockTime time.Time,
	mineTo []wire.TxOut) (*luxutil.Block, error) {
	...
	prevBlock := luxutil.NewBlock(mBlock)
