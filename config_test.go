...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-luxd.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "luxd")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of luxd binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-luxd.conf")
