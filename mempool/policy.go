...
const (
	...
	DefaultMinRelayTxFee = luxutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee luxutil.Amount) int64 {
	...
	if minFee < 0 || minFee > luxutil.MaxSatoshi {
		minFee = luxutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *luxutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee luxutil.Amount) bool {
...
func checkTransactionStandard(tx *luxutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee luxutil.Amount,
...
func GetTxVirtualSize(tx *luxutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
