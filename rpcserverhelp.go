...
var helpDescsEnUS = map[string]string{
	...
	"getblock-verbosetx":   "Specifies that each transaction is returned as a JSON object and only applies if the verbose flag is true (luxd extension)",
	...
	"sendrawtransaction-allowhighfees": "Whether or not to allow insanely high fees (luxd does not yet implement this parameter, so it has no effect)",
	...
	// StopCmd help.
	"stop--synopsis": "Shutdown luxd.",
	"stop--result0":  "The string 'luxd stopping.'",
	...
	// NotifySpentCmd help.
	"notifyspent--synopsis": "Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this luxd instance) and when such a transaction first appears in a newly-attached block.",
