...
Bitcoin RPC server that uses a luxd/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with luxd (https://github.com/bitbandi/luxd),
btcwallet (https://github.com/btcsuite/btcwallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, luxd and
...
luxd or btcwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by luxd and
...
All notifications provided by luxd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by luxd (and
...
Also, it is important to realize that luxd intentionally separates the wallet
functionality into a separate process named btcwallet.  This means if you are
connected to the luxd RPC server directly, only the RPCs which are related to
...
The following full-blown client examples are in the examples directory:
 ...
 - luxdwebsockets
   Connects to a luxd RPC server using TLS-secured websockets, registers for
