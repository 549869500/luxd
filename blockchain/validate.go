...
const (
	...
	baseSubsidy = 50 * luxutil.SatoshiPerBitcoin
)
...
func IsCoinBase(tx *luxutil.Tx) bool {
...
func IsFinalizedTransaction(tx *luxutil.Tx, blockHeight int32, blockTime time.Time) bool {
	msgTx := tx.MsgTx()
...
func CheckTransactionSanity(tx *luxutil.Tx) error {
	...
	for _, txOut := range msgTx.TxOut {
		...
		if totalSatoshi > luxutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
// difficulty is in min/max range and that the block hash is less than the
// target difficulty as claimed.
func CheckProofOfWork(block *luxutil.Block, powLimit *big.Int) error {
...
// txscript.
func CountSigOps(tx *luxutil.Tx) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *luxutil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
func checkBlockSanity(block *luxutil.Block, powLimit *big.Int, timeSource MedianTimeSource, flags BehaviorFlags) error {
...
// CheckBlockSanity performs some preliminary checks on a block to ensure it is
// sane before continuing with block processing.  These checks are context free.
func CheckBlockSanity(block *luxutil.Block, powLimit *big.Int, timeSource MedianTimeSource) error {
...
func ExtractCoinbaseHeight(coinbaseTx *luxutil.Tx) (int32, error) {
...
func checkSerializedHeight(coinbaseTx *luxutil.Tx, wantHeight int32) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkBlockContext(block *luxutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
	// The genesis block is valid by definition.
	if prevNode == nil {
		return nil
	}
...
func (b *BlockChain) checkBIP0030(node *blockNode, block *luxutil.Block, view *UtxoViewpoint) error {
...
func CheckTransactionInputs(tx *luxutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		if originTxSatoshi < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", luxutil.Amount(originTxSatoshi))
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxSatoshi > luxutil.MaxSatoshi {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				luxutil.Amount(originTxSatoshi),
				luxutil.MaxSatoshi)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		if totalSatoshiIn < lastSatoshiIn ||
			totalSatoshiIn > luxutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block *luxutil.Block, view *UtxoViewpoint, stxos *[]spentTxOut) error {
...
func (b *BlockChain) CheckConnectBlock(block *luxutil.Block) error {
	b.chainLock.Lock()
